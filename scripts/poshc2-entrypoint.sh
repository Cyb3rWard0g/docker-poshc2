...
if [[ "$BIND_IP" ]]; then
  ...
  sed -i "s/^BindIP:.*$/BindIP: '${BIND_IP}'/g" ${SERVER_CONFIG}/config.yml
fi

# BindPort: 443
if [[ "$BIND_PORT" ]]; then
  echo "[+] Setting PoshC2 Bind port to ${BIND_PORT}"
  sed -i "s/^BindPort:.*$/BindPort: ${BIND_PORT}/g" ${SERVER_CONFIG}/config.yml
fi
...
if [[ "$POSH_INSTALL_DIRECTORY" ]]; then
  ...
  sed -i "s|^PoshInstallDirectory:.*$|PoshInstallDirectory: \"${POSH_INSTALL_DIRECTORY}\"|g" ${SERVER_CONFIG}/config.yml
fi
...
if [[ "$POSH_PROJECT_DIRECTORY" ]]; then
  ...
  sed -i "s|^PoshProjectDirectory:.*$|PoshProjectDirectory: \"${POSH_PROJECT_DIRECTORY}\"|g" ${SERVER_CONFIG}/config.yml
fi
...
if [[ "$DATABASE_TYPE" ]]; then
  ...
  sed -i "s/^DatabaseType:.*$/DatabaseType: ${DATABASE_TYPE}/g" ${SERVER_CONFIG}/config.yml
fi
...
if [[ "$POSTGRES_CONNECTION_STRING" ]]; then
  ...
  sed -i "s/^PostgresConnectionString:.*$/PostgresConnectionString: \"${POSTGRES_CONNECTION_STRING}\"/g" ${SERVER_CONFIG}/config.yml
fi
...
if [[ "$PAYLOAD_COMMS_HOST" ]]; then
  ...
  sed -i "s|^PayloadCommsHost:.*$|PayloadCommsHost: \"${PAYLOAD_COMMS_HOST}\"|g" ${SERVER_CONFIG}/config.yml
fi

# PayloadCommsPort: 443
if [[ "$PAYLOAD_COMMS_PORT" ]]; then
  echo "[+] Setting PoshC2 PayloadCommsPort to ${PAYLOAD_COMMS_PORT}"
  sed -i "s/^PayloadCommsPort:.*$/PayloadCommsPort: ${PAYLOAD_COMMS_PORT}/g" ${SERVER_CONFIG}/config.yml
fi
...
if [[ "$DOMAIN_FRONT_HEADER" ]]; then
  ...
  sed -i "s/^DomainFrontHeader:.*$/DomainFrontHeader: \"${DOMAIN_FRONT_HEADER}\"/g" ${SERVER_CONFIG}/config.yml
fi
...
if [[ "$REFERRER" ]]; then
  ...
  sed -i "s/^Referrer:.*$/Referrer: \"${REFERRER}\"/g" ${SERVER_CONFIG}/config.yml
fi

# ServerHeader: "Apache"
if [[ "$SERVER_HEADER" ]]; then
  echo "[+] Setting PoshC2 ServerHeader to ${SERVER_HEADER}"
  sed -i "s/^ServerHeader:.*$/ServerHeader: \"${SERVER_HEADER}\"/g" ${SERVER_CONFIG}/config.yml
fi
...
if [[ "$USER_AGENT" ]]; then
  ...
  sed -i "s/^UserAgent:.*$/UserAgent: \"${USER_AGENT}\"/g" ${SERVER_CONFIG}/config.yml
fi

# DefaultSleep: "5s"
if [[ "$DEFAULT_SLEEP" ]]; then
  echo "[+] Setting PoshC2 DefaultSleep to ${DEFAULT_SLEEP}"
  sed -i "s/^DefaultSleep:.*$/DefaultSleep: \"${DEFAULT_SLEEP}\"/g" ${SERVER_CONFIG}/config.yml
fi

# Jitter: 0.20
if [[ "$JITTER" ]]; then
  echo "[+] Setting PoshC2 Jitter to ${JITTER}"
  sed -i "s/^Jitter:.*$/Jitter: ${JITTER}/g" ${SERVER_CONFIG}/config.yml
fi
...
if [[ "$KILL_DATE" ]]; then
  ...
  sed -i "s/^KillDate:.*$/KillDate: \"${KILL_DATE}\"/g" ${SERVER_CONFIG}/config.yml
fi
...
if [[ "$URL_CONFIG" ]]; then
  ...
  sed -i "s/^UrlConfig:.*$/UrlConfig: \"${URL_CONFIG}\"/g" ${SERVER_CONFIG}/config.yml
fi
...
if [[ "$DEFAULT_MIGRATION_PROCESS" ]]; then
  ...
  sed -i "s/^DefaultMigrationProcess:.*$/DefaultMigrationProcess: \"${DEFAULT_MIGRATION_PROCESS}\"/g" ${SERVER_CONFIG}/config.yml
fi
...
if [[ "$SOUNDS" ]]; then
  ...
  sed -i "s/^Sounds:.*$/Sounds: \"${SOUNDS}\"/g" ${SERVER_CONFIG}/config.yml
fi
...
if [[ "$NOTIFICATIONS_PROJECT_NAME" ]]; then
  ...
  sed -i "s/^NotificationsProjectName:.*$/NotificationsProjectName: \"${NOTIFICATIONS_PROJECT_NAME}\"/g" ${SERVER_CONFIG}/config.yml
fi

# EnableNotifications: "No"
if [[ "$ENABLE_NOTIFICATIONS" ]]; then
  echo "[+] Setting PoshC2 EnableNotifications to ${ENABLE_NOTIFICATIONS}"
  sed -i "s/^EnableNotifications:.*$/EnableNotifications: \"${ENABLE_NOTIFICATIONS}\"/g" ${SERVER_CONFIG}/config.yml
fi
...
if [[ "$PUSHOVER_APITOKEN" ]]; then
  ...
  sed -i "s/^Pushover_APIToken:.*$/Pushover_APIToken: \"${PUSHOVER_APITOKEN}\"/g" ${SERVER_CONFIG}/config.yml
fi

# Pushover_APIUser: ""
if [[ "$PUSHOVER_APIUSER" ]]; then
  echo "[+] Setting PoshC2 Pushover_APIUser to ${PUSHOVER_APIUSER}"
  sed -i "s/^Pushover_APIUser:.*$/Pushover_APIUser: \"${PUSHOVER_APIUSER}\"/g" ${SERVER_CONFIG}/config.yml
fi
...
if [[ "$SOCKS_HOST" ]]; then
  ...
  sed -i "s|^SocksHost:.*$|SocksHost: \"${SOCKS_HOST}\"|g" ${SERVER_CONFIG}/config.yml
fi
